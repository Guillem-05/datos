...
$edadHermano1 = intval(readline("Ingrese la edad del primer hermano: "));
$edadHermano2 = intval(readline("Ingrese la edad del segundo hermano: "));
$diferencia = diferenciaEdad($edadHermano1, $edadHermano2);
echo "La diferencia de edad es: $diferencia años.\n";
...
function numRomano($numero) {
    ...
    return $romanos[$numero] ?? "Número fuera de rango";
}

$numero = intval(readline("Ingrese un número entre 1 y 10: "));
$romano = numRomano($numero);
echo "El número $numero en romano es: $romano\n";
...
sort($numeros);
echo "Combinación de lotería: " . implode(", ", $numeros) . "\n";
...
foreach ($empleados as $nombre => $info) {
    $horas = $info['horas'];
    $tarifa = $info['tarifa'] ?? 10;
    $salario = calculaSalario($horas, $tarifa);
    echo "$nombre cobrará: €$salario\n";
}
...
function edadHumana($edad) {
    if ($edad <= 2) return $edad * 10.5;
    if ($edad <= 6) return 21 + ($edad - 2) * 4;
    return 21 + ($edad - 6) * 5;
}

function sobrePeso(&$data) {
    $sobrepesoCount = 0;
    foreach ($data as $perro) {
        $edadHumana = edadHumana($perro['edad']);
        if (($edadHumana >= 60 && $perro['peso'] > 7) || 
            ($edadHumana < 40 && $perro['peso'] > 3) || 
            ($edadHumana >= 40 && $edadHumana < 60 && $perro['peso'] > 6)) {
            $sobrepesoCount++;
        }
    }
    return $sobrepesoCount;
}

$totalPerros = count($data);
$sobrepeso = sobrePeso($data);
$porcentajeSobrepeso = ($sobrepeso / $totalPerros) * 100;
echo "Porcentaje de perros con sobrepeso: $porcentajeSobrepeso%\n";
...
$caracter = readline("Ingrese un carácter: ");
$numero = intval(readline("Ingrese un número: "));
figura($caracter, $numero);
...
function buscarPalabra($texto, $palabra) {
    ...
    return $coincidencias > 0 ? "La palabra '$palabra' existe $coincidencias veces." : "La palabra '$palabra' no existe.";
}
...
$texto = "Este es un texto de ejemplo para buscar y reemplazar palabras.";
...
function analizarTexto($texto) {
    $numCaracteres = strlen(str_replace([" ", "\n", "\r"], '', $texto));
    $numPalabras = str_word_count($texto);
    $numFrases = substr_count($texto, '.') + substr_count($texto, '!') + substr_count($texto, '?');
    $numParrafos = substr_count($texto, "\n\n") + 1;
    $letrasPorPalabra = $numCaracteres / $numPalabras;
    $palabrasPorFrase = $numPalabras / $numFrases;
    $frasesPorParrafo = $numFrases / $numParrafos;

    return [
        'numCaracteres' => $numCaracteres,
        'numPalabras' => $numPalabras,
        'numFrases' => $numFrases,
        'numParrafos' => $numParrafos,
        'letrasPorPalabra' => $letrasPorPalabra,
        'palabrasPorFrase' => $palabrasPorFrase,
        'frasesPorParrafo' => $frasesPorParrafo,
    ];
}

$texto = "Este es un texto de ejemplo. Aquí hay otra frase. Y un párrafo más.";
